...
describe('ScoreManager', () => {
    ...
    test('handles errors when reading scores from file', () => {
        fs.readFileSync.mockImplementationOnce(() => { throw new Error('Failed to read'); });
    ...
    test('saves scores to file', () => {
        const newScore = { name: "Charlie", score: 25, emoji: "🤣", date: new Date().toISOString() };
    ...
    test('does not add score if it is zero', () => {
        const zeroScore = { name: "Zero", score: 0, emoji: "😑", date: new Date().toISOString() };
